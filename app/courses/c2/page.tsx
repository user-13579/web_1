...
export default function CourseC2Page() {
  const title = 'Advanced Nutrition';
  const about = 'Deep-dive into macros, micros, and hydration strategies.';
  const youtubeId = 'ysz5S6PUM-U';
  const coverIdx = 2;
  const { user, purchases, setPurchased } = useAuth();
  const hasAccess = !!purchases.courses || !!purchases['course_c2'];
  ...
  useEffect(() => {
    ...
    if (product === 'course') {
      (async () => {
        try {
          await setPurchased('course_c2' as any, true);
              ...
              onClick={async () => {
                try {
                  const res = await fetch('/api/checkout/course', {
                    ...
                    body: JSON.stringify({ uid: user?.uid, email: user?.email, courseId: 'c2' }),
                  });
      ...
      <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <div className="rounded-2xl border border-neutral-200/70 bg-white p-4 text-sm text-neutral-700">
          ...
          <ul className="list-disc pl-5">
            <li>Understanding macro and micronutrients</li>
            <li>Building balanced meal plans</li>
            <li>Hydration strategies for daily life</li>
          </ul>
        </div>
      </div>
