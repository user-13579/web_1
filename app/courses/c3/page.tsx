...
export default function CourseC3Page() {
  const title = 'Quick Weeknight Dinners';
  const about = 'Fast, flavorful dinners using minimal tools.';
  const youtubeId = 'aqz-KE-bpKQ';
  const coverIdx = 3;
  const { user, purchases, setPurchased } = useAuth();
  const hasAccess = !!purchases.courses || !!purchases['course_c3'];
  ...
  useEffect(() => {
    ...
    if (product === 'course') {
      (async () => {
        try {
          await setPurchased('course_c3' as any, true);
              ...
              onClick={async () => {
                try {
                  const res = await fetch('/api/checkout/course', {
                    ...
                    body: JSON.stringify({ uid: user?.uid, email: user?.email, courseId: 'c3' }),
                  });
      ...
      <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <div className="rounded-2xl border border-neutral-200/70 bg-white p-4 text-sm text-neutral-700">
          ...
          <ul className="list-disc pl-5">
            <li>Speedy prep techniques</li>
            <li>One-pan/one-pot approaches</li>
            <li>Batching and freezer-ready meals</li>
          </ul>
        </div>
      </div>
